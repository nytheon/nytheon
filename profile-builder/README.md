...
- Builds `README.md` entirely from live services so every number updates in
  real time: `github-readme-stats` (served via the live mirror deployment),
  `skillicons.dev` real language logos, `streak-stats`,
  `github-readme-activity-graph` and `img.shields.io`.
