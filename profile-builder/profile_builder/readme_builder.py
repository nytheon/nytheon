...
- stats .......... github-readme-stats.vercel.app
...
Only local, generated assets referenced: assets/banner.svg (branding).
"""
...
def trophy_url(cfg: ProfileConfig) -> str:
    return (
        "https://github-profile-trophy.vercel.app/?username="
        f"{cfg.username}&theme={cfg.trophy_theme}&no-bg=true&no-frame=true"
        "&row=1&column=6"
    )
...
def _stats(cfg: ProfileConfig) -> List[str]:
    return [
        ...
        '<div align="center">',
        "",
        f'  <img src="{trophy_url(cfg)}" alt="GitHub trophies"/>',
        "",
        "</div>",
        "",
    ]
...
def write_assets(cfg: ProfileConfig, live: LiveData, root: str) -> List[str]:
    ...
    Only the hero banner (and divider) are written here; everything else
    in the README is live and needs no files on disk.
    """
